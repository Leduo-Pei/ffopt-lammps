...
FFOpt 是一个面向材料的通用力场开发框架。本手册介绍共用的安装、输入和
运行方法，并以 BTAH 演示分子体系的操作。用户准备 LAMMPS data 文件、
实验目标和合理的初始参数范围，编辑一个 `ffopt.in`，即可组织参数搜索、
采样、代理模型训练、主动学习和最终 LAMMPS 验证，并利用已保存的状态续算。

> 当前版本为 alpha。目前已经开展分子晶体和单质体心立方（BCC）材料两类
> 工作流；BCC 是 FFOpt 内部的材料扩展，不是另一套软件。BTAH 是内置的
> 分子回归体系，Fe 是内置的 BCC 示例。通用框架的定位不等于已经验证了
> 所有材料、合金、反应力场或多晶型迁移。
> 当前吸附后端把一个指定的零电荷金属 type 视为固定基底，只优化分子 types；
> 带电、多组分或也需要优化参数的基底不属于 schema 1 的支持范围。

BCC 用户请结合 [BCC 计算指南](../how-to/elemental-bcc.md)和
[带注释的 Fe 输入](../../examples/fe_bcc/ffopt.in)使用。该路径先满足结构
约束，再优化弹性性质；内置示例使用高斯过程，而非分子示例中的 ANN。
双 type 的有序亚晶格 LJ 模型也不等于可迁移到任意场景的单质势，适用范围
与独立验证要求见上述指南。

## 1. 先理解三个文件层次

一个项目只需要保留下面的结构。以下以分子性质模块为例；BCC 使用相应的
体相、表面和弹性输入：
